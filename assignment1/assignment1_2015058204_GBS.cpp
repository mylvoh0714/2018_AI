...
bool GreedyBFS(int row, int col)
{
	int dist = getHeuristic(row, col, ExitArr);
	Point p = { row,col };
	pq.push({ dist, p });
	visited[row][col] = 1;
	while ( !pq.empty() )
	{
		...
		for ( int i = 0; i < 4; i++ )
		{
			...
			{
				Point p_there = { my,mx };
				int dist_there = getHeuristic(my, mx, ExitArr);
				
				//cout << "Greedy BFS 이동경로 : (" << p_here.row << "," <<  p_here.col << ")" 
